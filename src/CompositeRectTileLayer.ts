...
export class CompositeRectTileLayer extends Container {
  ...
  setBitmaps(bitmaps: Array<Texture>) {
    ...
    let i: number;
    for (i = 0; i < len1; i++) {
      (this.children[i] as RectTileLayer).textures = bitmaps.slice(
        i * texPerChild,
        (i + 1) * texPerChild
      );
    }
  ...
  addRect(
    ...
    if (
      ...
      (this.children[childIndex] as RectTileLayer).textures
    ) {
  ...
  addFrame(
    ...
    let texture: Texture;
    let layer: RectTileLayer | null = null;
    let ind: number = 0;
